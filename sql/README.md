...
`build_sqlite.py` loads both published CSVs into `sql/ghana.db`. That file is a build artefact and is not committed — the CSVs are the source of truth, and the database is a second away from them.
...
|---|---|---|
| `01_schema.sql` | — | Two tables mirroring the two CSVs |
| `02_load.sql` | — | CSV import for the sqlite3 shell |
